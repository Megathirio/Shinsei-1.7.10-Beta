...
public class BlockScarletEmeraldLamp extends BlockShinsei {

    private final boolean boolOn;

    public BlockScarletEmeraldLamp(boolean boolOn) {
        ...
        if (boolOn) {
            ...
            this.setBlockName(Names.Materials.SCARLET_EMERALD + Names.Forms.LAMP + Names.States.ON);
            this.setCreativeTab(null);
        }else{
            this.setBlockName(Names.Materials.SCARLET_EMERALD + Names.Forms.LAMP + Names.States.OFF);
    ...
    public void onBlockAdded(World world, int intX, int intY, int intZ) {
        ...
        if (!world.isRemote) {
            if (this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
                ...
                world.setBlock(intX, intY, intZ, ShinseiBlocks.scarletEmeraldLampOn, 0, 2);
            }
        }
    }
    ...
    public void updateTick(World world, int intX, int intY, int intZ, Random random) {
        ...
        if (!world.isRemote && this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
            world.setBlock(intX, intY, intZ, ShinseiBlocks.scarletEmeraldLampOff, 0, 2);
        }
    }

    public Item getItemDropped(int intI, Random random, int intJ){
        return Item.getItemFromBlock(ShinseiBlocks.scarletEmeraldLampOff);
    }

    protected ItemStack createStackedBlock(int intI){
        return new ItemStack(ShinseiBlocks.scarletEmeraldLampOff);
    }
