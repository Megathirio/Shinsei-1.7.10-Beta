...
public class BlockEmeraldLamp extends BlockShinsei {

    private final boolean boolOn;

    public BlockEmeraldLamp(boolean boolOn) {
        ...
        if (boolOn) {
            ...
            this.setBlockName(Names.Materials.EMERALD + Names.Forms.LAMP + Names.States.ON);
            this.setCreativeTab(null);
        }else{
            this.setBlockName(Names.Materials.EMERALD + Names.Forms.LAMP + Names.States.OFF);
    ...
    public void onBlockAdded(World world, int intX, int intY, int intZ) {
        ...
        if (!world.isRemote) {
            if (this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
                ...
                world.setBlock(intX, intY, intZ, ShinseiBlocks.emeraldLampOn, 0, 2);
            }
        }
    }
    ...
    public void updateTick(World world, int intX, int intY, int intZ, Random random) {
        ...
        if (!world.isRemote && this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
            world.setBlock(intX, intY, intZ, ShinseiBlocks.emeraldLampOff, 0, 2);
        }
    }

    public Item getItemDropped(int intI, Random random, int intJ){
        return Item.getItemFromBlock(ShinseiBlocks.emeraldLampOff);
    }

    protected ItemStack createStackedBlock(int intI){
        return new ItemStack(ShinseiBlocks.emeraldLampOff);
    }
