...
public class BlockAquamarineLamp extends BlockShinsei {

    private final boolean boolOn;

    public BlockAquamarineLamp(boolean boolOn) {
        ...
        if (boolOn) {
            ...
            this.setBlockName(Names.Materials.AQUAMARINE + Names.Forms.LAMP + Names.States.ON);
            this.setCreativeTab(null);
        }else{
            this.setBlockName(Names.Materials.AQUAMARINE + Names.Forms.LAMP + Names.States.OFF);
    ...
    public void onBlockAdded(World world, int intX, int intY, int intZ) {
        ...
        if (!world.isRemote) {
            if (this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
                ...
                world.setBlock(intX, intY, intZ, ShinseiBlocks.aquamarineLampOn, 0, 2);
            }
        }
    }
    ...
    public void updateTick(World world, int intX, int intY, int intZ, Random random) {
        ...
        if (!world.isRemote && this.boolOn && !world.isBlockIndirectlyGettingPowered(intX, intY, intZ)) {
            world.setBlock(intX, intY, intZ, ShinseiBlocks.aquamarineLampOff, 0, 2);
        }
    }

    public Item getItemDropped(int intI, Random random, int intJ){
        return Item.getItemFromBlock(ShinseiBlocks.aquamarineLampOff);
    }

    protected ItemStack createStackedBlock(int intI){
        return new ItemStack(ShinseiBlocks.aquamarineLampOff);
    }
